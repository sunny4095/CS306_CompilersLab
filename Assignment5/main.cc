...
#include "tac.hh"
#include "rtl.hh"
#include "asm.hh"             // Added for Assembly generation
#include "symbol_table.hh"    // Added to resolve the SymbolTable type
extern int yyparse();
extern int yylex();
extern FILE *yyin;
extern int yylineno;
...
int main(int argc, char *argv[]) {
    ...
    TAC_Code tac_code = TAC_Code();
    RTL_Code rtl_code = RTL_Code();
    Asm_Code asm_code = Asm_Code(&symtab);
    ast_root->gen_tac_code(tac_code);
    tac_code.gen_rtl_code(rtl_code);
    rtl_code.gen_asm_code(asm_code);
    string rtl_file = new_file_name(input_file, ".rtl");
    string asm_file = new_file_name(input_file, ".spim");
    ofstream out(rtl_file);
    ofstream asm_out(asm_file);
    tac_code.print();
    rtl_code.print(out);
    asm_code.print(asm_out);
    fclose(yyin);
    if (show_tokens) fclose(fp);
